 ...
 #                               raw_input("ENd of Bisect")
    ...
    def fold(self, poly, foldline, toFold,SearchNode = None, pHang = False):        
       ...
       # for s in self.shapes:
        ...
        for p in halves:
            if p != False:
                if (len(p.vertices()) <= 2):
                    print "ERROR: too few vertices in fold  Poly is" , p, halves, poly, foldline.start(), foldline.end() 
                ...
                #    # printpoly,  self.isFoldPerpendicularToHang(poly, foldline)
                  ...
                  #              print "Which poly pt is added", poly, pHang
                    ...
                    if (len(drawp.vertices()) <=2):
                        print "ERROR num vertices: drawp first" , drawp
                        raw_input()
                    ...
                    if poly.isHang(): #and not self.isFoldPerpendicularToHang(poly,foldline, toFold):    #check if mirroredPoly is still hanging
                        ...
                        for b in bisected:
                            ...
                            if (b==False):
                                ...
                                if (len(b.vertices()) <=2):
                                    print "ERROR num vertices: drawp" , b
                                    raw_input()
                                ...
                                if not self.isPolyHanging(b, direc):
                                    ...
                                    drawp = b
                                    if (len(drawp.vertices() <=2):
                                        print "ERROR num vertices: drawp bisect else" , drawp
                                        raw_input()
                        ...
                        cvpoly = CVPolygon(drawc,drawh,drawp)
                        if (drawp.vertices() <=2):
                            print "ERROR num vertices: drawp else" , drawp
                            raw_input()
    ...
    def drag(self, poly, foldline, toFold, direction, distance, SearchNode = None, pHang = False):
        ...
        for p in halves:
            if p != False:
                if(p.vertices() <=2):
                    print "P in halves too few vertices in drag", p, poly, halves, foldline.start(), foldline.end()
                ...
                if len([folded for folded in toFold if folded.containsExclusive(p.randPt())]) > 0:
                    ...
                    drawp = Geometry2D.movePoly(p,direction,distance)
                    if (drawp.vertices() <=2):
                        print "ERROR num vertices: drawp drag" , drawp
                        raw_input()
                    drawh = height
                    drawc = color
                    tableEdge = self.getClosestTableEdge(direction)
                    ...
                    for b in bisected:
                        if b!=False:
                            ...
                            if self.isPolyHanging(b, direction):
                                if (b.vertices() <=2):
                                    print "ERROR num vertices: b in dragbisect" ,b 
                                    raw_input()
                                drawc = Colors.lightenCV(color, 0.40)
                                cvpoly = CVPolygon(drawc,drawh,b)
                                cvpoly.setHang(True, direction)
                                ...
                                drawp = b
                                if (drawp.vertices() <=2):
                                    print "ERROR num vertices: b in dragBisect else" , b
                                    raw_input()
                                drawh = height
                                drawc = color #Colors.lightenCV(color,50)
                                cvpoly = CVPolygon(drawc,drawh,drawp)
                    ...
                    drawp = Geometry2D.movePoly(p,direction,distance)
                    if (drawp.vertices() <=2):
                        print "ERROR num vertices: drawp IN DRAG" , drawp
                        raw_input()
                    drawh = height
                    drawc = color
