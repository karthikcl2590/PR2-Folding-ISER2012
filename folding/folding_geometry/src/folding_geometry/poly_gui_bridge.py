...
class PolyGUIBridge():
    ...
    def poly_handler(self,stamped_poly):
        ...
        util.poly_frame = "table_front" #stamped_poly.header.frame_id
    ...
    def handle_automatic_folds(self,vertices):                        
        vertices = [self.convert_to_world_frame(vertex) for vertex in vertices]
        if len(vertices) == 10 and self.mode == "shirt":
            ...
            self.robot.arms_test([bl,br],[tl,tr])
            self.stop_logging()
